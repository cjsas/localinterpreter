...
def main(cfg: SalmEvalConfig):
    ...
    for batch_idx, batch in enumerate(dloader):
        # Calculate total audio duration in this batch
        batch_audio_duration = sum(batch["audio_lens"].float() / 16000.0)  # Assuming 16kHz sample rate
    ...
    rtfx_score = total_audio_duration / sum(infer_durations) if sum(infer_durations) > 0 else 0
    tps = sum(num_answer_tokens) / sum(infer_durations)
    logging.info(f"TPS: {tps:.2f}")
    logging.info(f"Overall RTFX: {rtfx_score:.2f}x")
